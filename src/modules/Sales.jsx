...
export function SalesTrendAnalysis() {
  ...
  // Exclude current in-progress month if its lead count is partial (< 25% of recent average)
  const fullMonths = sortedMonths.filter((t, idx) => {
    if (idx === sortedMonths.length - 1 && sortedMonths.length > 1) {
      const avgPrev = sortedMonths.slice(0, -1).reduce((s, x) => s + x.leads, 0) / (sortedMonths.length - 1);
      return t.leads >= avgPrev * 0.25;
    }
    return true;
  });

  const anchorMonths = fullMonths.length ? fullMonths : sortedMonths;
  const recentAnchor = anchorMonths.slice(-3);

  // Baseline lead volume for full-month projections
  const baseLeads = recentAnchor.length ? Math.round(recentAnchor.reduce((s, t) => s + t.leads, 0) / recentAnchor.length) : 100;

  // Baseline weighted conversion rate across anchor months
  const anchorTotalLeads = recentAnchor.reduce((s, t) => s + t.leads, 0);
  const anchorTotalWon = recentAnchor.reduce((s, t) => s + t.won, 0);
  const baseConvPct = anchorTotalLeads > 0 ? Math.round((anchorTotalWon / anchorTotalLeads) * 1000) / 10 : 45;

  // Monthly growth momentum (+4% per month projection curve)
  const growthRate = 0.04;

  const forecastMonths = [];
  if (sortedMonths.length) {
    let { y, m } = sortedMonths[sortedMonths.length - 1];
    let runningLeads = baseLeads;

    for (let i = 0; i < FORECAST_MONTHS; i++) {
      m += 1; if (m > 11) { m = 0; y += 1; }
      runningLeads = Math.round(runningLeads * (1 + growthRate));
      const projWon = Math.round(runningLeads * (baseConvPct / 100));

      forecastMonths.push({
        label: `${MONL[m]} '${String(y).slice(2)}`,
        leadsProjected: runningLeads,
        wonProjected: projWon,
        convProjected: baseConvPct,
      });
    }
  }
  ...
  const chartData = [
    ...
    ...forecastMonths.map(f => ({
      label: f.label,
      leadsActual: null,
      leadsProjected: f.leadsProjected,
      wonActual: null,
      wonProjected: f.wonProjected,
      convActual: null,
      convProjected: f.convProjected,
    })),
  ];
  ...
  const totalChannelLeads = channelStats.reduce((s, c) => s + c.leads, 0);
  const totalChannelWon = channelStats.reduce((s, c) => s + c.won, 0);
  const totalChannelWinRate = totalChannelLeads ? Math.round((totalChannelWon / totalChannelLeads) * 100) : 0;

  const totalSocLeads = topSocieties.reduce((s, b) => s + b.leads, 0);
  const totalSocWon = topSocieties.reduce((s, b) => s + b.won, 0);
  const totalSocRate = totalSocLeads ? Math.round((totalSocWon / totalSocLeads) * 100) : 0;

  const totalDropCount = dropReasons.reduce((s, r) => s + r.count, 0);
              ...
              <ComposedChart data={chartData} margin={{ top: 28, right: 28, left: 12, bottom: 12 }}>
                ...
                <Line yAxisId="leads" type="monotone" dataKey="leadsActual" name="Lead Volume" stroke="#08805A" strokeWidth={2.5} dot={{ r: 4, fill: "#08805A" }} isAnimationActive={false} connectNulls={false}>
                  <LabelList dataKey="leadsActual" content={(props) => {
                    ...
                    const lx = index === 0 ? x + 14 : index === chartData.length - 1 ? x - 14 : x;
                    return (
                      <g>
                        <text x={lx} y={y - 14} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}</text>
                        <text x={lx} y={y - 14} fill="#08805A" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}</text>
                      </g>
                    );
                  }} />
                </Line>
                <Line yAxisId="leads" type="monotone" dataKey="leadsProjected" name="Lead Volume (proj.)" stroke="#08805A" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3, fill: "#08805A" }} isAnimationActive={false} connectNulls={true}>
                  <LabelList dataKey="leadsProjected" content={(props) => {
                    ...
                    const lx = index === chartData.length - 1 ? x - 14 : x;
                    return (
                      <g>
                        <text x={lx} y={y - 14} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11} fontWeight={800} textAnchor="middle">{value}</text>
                        <text x={lx} y={y - 14} fill="#08805A" fontSize={11} fontWeight={800} textAnchor="middle">{value}</text>
                      </g>
                    );
                  }} />
                </Line>

                {/* Converted Leads (Actual & Projected) — HIG Electric Blue */}
                <Line yAxisId="leads" type="monotone" dataKey="wonActual" name="Converted Leads" stroke="#0284C7" strokeWidth={2.5} dot={{ r: 4, fill: "#0284C7" }} isAnimationActive={false} connectNulls={false}>
                  <LabelList dataKey="wonActual" content={(props) => {
                    ...
                    return (
                      <g>
                        <text x={lx} y={y + 18} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}</text>
                        <text x={lx} y={y + 18} fill="#0284C7" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}</text>
                      </g>
                    );
                  }} />
                </Line>
                <Line yAxisId="leads" type="monotone" dataKey="wonProjected" name="Converted Leads (proj.)" stroke="#0284C7" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3, fill: "#0284C7" }} isAnimationActive={false} connectNulls={true}>
                  <LabelList dataKey="wonProjected" content={(props) => {
                    ...
                    return (
                      <g>
                        <text x={lx} y={y + 18} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11} fontWeight={800} textAnchor="middle">{value}</text>
                        <text x={lx} y={y + 18} fill="#0284C7" fontSize={11} fontWeight={800} textAnchor="middle">{value}</text>
                      </g>
                    );
                  }} />
                </Line>

                {/* Conversion % (Actual & Projected) */}
                <Line yAxisId="conv" type="monotone" dataKey="convActual" name="Conversion %" stroke="#F59E0B" strokeWidth={2.5} dot={{ r: 4, fill: "#F59E0B", stroke: "#ffffff", strokeWidth: 1.5 }} isAnimationActive={false} connectNulls={false}>
                  <LabelList dataKey="convActual" content={(props) => {
                    const { x, y, value, index } = props;
                    if (value == null || value === "") return null;
                    const lx = index === 0 ? x + 18 : index === chartData.length - 1 ? x - 18 : x;
                    return (
                      <g>
                        <text x={lx} y={y - 14} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}%</text>
                        <text x={lx} y={y - 14} fill="#D97706" fontSize={11.5} fontWeight={800} textAnchor="middle">{value}%</text>
                      </g>
                    );
                  }} />
                </Line>
                <Line yAxisId="conv" type="monotone" dataKey="convProjected" name="Conversion % (proj.)" stroke="#D97706" strokeWidth={2} strokeDasharray="4 4" dot={{ r: 3, fill: "#D97706" }} isAnimationActive={false} connectNulls={true}>
                  <LabelList dataKey="convProjected" content={(props) => {
                    ...
                    return (
                      <g>
                        <text x={x - 18} y={y - 14} fill="none" stroke="#ffffff" strokeWidth={4} strokeLinejoin="round" paintOrder="stroke fill" fontSize={11} fontWeight={800} textAnchor="middle">{value}% (proj.)</text>
                        <text x={x - 18} y={y - 14} fill="#D97706" fontSize={11} fontWeight={800} textAnchor="middle">{value}% (proj.)</text>
                      </g>
                    );
                  }} />
                </Line>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid rgba(0,0,0,.06)", boxShadow: "0 10px 30px rgba(0,0,0,.03)", padding: 20 }}>
            <div style={{ marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ background: "rgba(220,65,65,0.08)", color: "#DC4141", padding: "4px 10px", borderRadius: 8, fontSize: 12, fontWeight: 700 }}>
                Total: {totalDropCount} leads
              </div>
            </div>
            ...
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.06)", paddingTop: 10, marginTop: 12, display: "flex", justifyContent: "space-between", fontSize: 12, fontWeight: 700, color: "#1D1D1F" }}>
              <span>Total Lost Leads</span>
              <span style={{ color: "#DC4141" }}>{totalDropCount} leads (100%)</span>
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid rgba(0,0,0,.06)", boxShadow: "0 10px 30px rgba(0,0,0,.03)", padding: 20 }}>
            <div style={{ marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ background: "rgba(8,128,90,0.08)", color: "#08805A", padding: "4px 10px", borderRadius: 8, fontSize: 12, fontWeight: 700 }}>
                Total: {totalChannelLeads} Leads
              </div>
            </div>
            <div className="scroll-thin" style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "center", fontSize: 13 }}>
                ...
                <tfoot>
                  <tr style={{ borderTop: "2px solid rgba(0,0,0,.08)", background: "rgba(243,248,236,.6)", fontWeight: 700 }}>
