...
const ZD_DEFAULT_STATUSES = ["Open", "In Progress", "On Hold", "Escalated", "Closed"];
...
// re-fetch). See src/lib/apiUsageTracker.js + notifyAdmin.js.
const API_USAGE_LIMITS = {
...
const apiTracker = new ApiUsageTracker({
  ...
  onThreshold: (threshold, status) => {
    ...
    notifyAdminEmail({
      apiOrigin: API_ORIGIN, authHeaders: authHeaders(), to: FAILURE_ALERT_TO,
      type: "api_usage_threshold",
      payload: { source: status.api, threshold, count: status.count, limit: status.limit },
    });
  },
});
const ipCache = makeCache("pw_ip_cache", 6 * 60 * 60 * 1000); // 6h — survives logout
...
const tkStatus = (label) => ({ label: label || "—", color: zdStatusColor(label) });
const tkPriority = (label) => ({ label: label || "—", color: zdPriorityColor(label) });
...
let PLAN_STATUSES = LS.get("pw_plan_statuses", null) || PLAN_STATUSES_DEFAULT;
let PLAN_SPRINTS = LS.get("pw_plan_sprints", null) || PLAN_SPRINTS_DEFAULT;
let PLAN_CATEGORIES = LS.get("pw_plan_categories", null) || PLAN_CATEGORIES_DEFAULT;
...
const planStatusMeta = (k) => PLAN_STATUSES.find(s => s.key === k) || PLAN_STATUSES[0];
const planPrioMeta = (k) => PLAN_PRIORITIES.find(p => p.key === k) || PLAN_PRIORITIES[1];
const planInitials = (name) => String(name || "?").split(" ").map(s => s[0]).slice(0, 2).join("").toUpperCase();
const planUserColor = (name) => { const i = Math.max(0, PLAN_USERS.indexOf(name)); return PLAN_AVATAR_COLORS[i % PLAN_AVATAR_COLORS.length]; };
const planReadFile = (file) => new Promise((resolve, reject) => {
...
async function planDownloadAttachment(a) {
  let href = a.url;
  if (!href) {
...
const AssigneeStack = ({ names, size = 24 }) => {
  const list = names || [];
...
const planPrio = (p) => PLAN_PRIO_MIGRATE[p] || (PLAN_PRIORITIES.some(x => x.key === p) ? p : "P2");
...
function TaskWeeklyView({ tasks, onOpen, isOverdue }) {
  const [expanded, setExpanded] = useState(() => new Set());
...
function TaskCard({ t, overdue, onOpen, onDragStart, onDragEnd, dragging }) {
...
function TaskEditor({ task, onClose, onSave, onDelete, onWarn }) {
...
const inp = { width: "100%", padding: "10px 12px", border: "1.5px solid var(--border)", borderRadius: 11, fontSize: 14, color: "var(--f)", background: "#fff", outline: "none", marginBottom: 0, fontFamily: "'DM Sans',system-ui,-apple-system,'Segoe UI',Roboto,sans-serif" };
