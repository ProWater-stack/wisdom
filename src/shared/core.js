...
// Canonical status normalizer (v2.29.365): merges variants like "Active" and "active"
// into canonical "Active" across filters, tables, and dropdowns.
export function canonicalStatus(s) {
  if (!s) return "";
  const str = String(s).trim();
  if (str.toLowerCase() === "active") return "Active";
  return str;
}
    ...
    const zohoCustomers = allRaw.map(c => {
      ...
      return {
        ...
        status:
          ...
          : (String(p.subscription_status || "").toLowerCase() === "active" ? "Active" : (p.subscription_status || "inactive")),
        ...
        deviceStatus:              p.device_status ?? c.device_status ?? p.dp_details?.device_status ?? c.dp_details?.device_status ?? "",
        device_status:             p.device_status ?? c.device_status ?? p.dp_details?.device_status ?? c.dp_details?.device_status ?? "",
        oldPurifierId:             p.old_purifier_id ?? c.old_purifier_id ?? p.dp_details?.old_purifier_id ?? "",
        old_purifier_id:           p.old_purifier_id ?? c.old_purifier_id ?? p.dp_details?.old_purifier_id ?? "",
        deviceReplacementReason:   p.device_replacement_reason ?? c.device_replacement_reason ?? p.dp_details?.device_replacement_reason ?? "",
        device_replacement_reason: p.device_replacement_reason ?? c.device_replacement_reason ?? p.dp_details?.device_replacement_reason ?? "",
        deviceReplacedFlag:        p.device_replaced_flag ?? c.device_replaced_flag ?? p.dp_details?.device_replaced_flag ?? "",
        device_replaced_flag:      p.device_replaced_flag ?? c.device_replaced_flag ?? p.dp_details?.device_replaced_flag ?? "",
...
// First name only — splits on a space OR a dot, so both "Arjun Marri" and a
// dot-separated stored name like "Arjun.Marri" (no space to split on) both
// correctly reduce to just "Arjun" rather than showing the whole string.
export const firstNameOf = (s) => { const w = String(s || "").trim().split(/[\s.]+/)[0]; return w ? w.charAt(0).toUpperCase() + w.slice(1) : w; };
...
export function mapInvoice(iv) {
  ...
  return {
    ...
    // intervalCount/intervalUnit (v2.29.369) — mirrors mapSubscription()'s own
    // split fields. `interval` above mixes unit-string and numeric-count
    // sources together (Zoho invoice records carry BOTH a numeric `interval`
    // — e.g. 12 — and a separate `interval_unit` string — e.g. "months" —
    // as distinct keys), so a real numeric term count gets silently masked
    // whenever a unit string is also present. Kept as separate fields so the
    // real numeric term survives for termMonths() to use directly, instead of
    // falling back to parsing a "12M"-style token out of the plan name/code.
    intervalCount:  Number(p.interval) || null,
    intervalUnit:   p.interval_unit || p.billing_interval || "",
